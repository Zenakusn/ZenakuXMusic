...
from Zenaku.config import (AUTO_DOWNLOADS_CLEAR, BANNED_USERS,
...
from Zenaku.misc import SUDOERS, db
from Zenaku.utils.database import (is_active_chat,
...
from Zenaku.utils.formatters import seconds_to_min
from Zenaku.utils.inline.play import (stream_markup,
    stream_markup_timer,
    telegram_markup,
    telegram_markup_timer,
    close_keyboard)
from Zenaku.utils.stream.autoclear import auto_clean
from Zenaku.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Zenakuh.pause_stream(chat_id)
        ...
        await Zenakuh.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Zenakuh.stop_stream(chat_id)
        ...
        await Zenakuh.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Zenakuh.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if "live_" in queued:
            ...
            try:
                await Zenakuh.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Zenakuh.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
